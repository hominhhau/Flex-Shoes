...
const ProfileForm = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (response.status === 200 && response.data && response.data.status === 'SUCCESS' && response.data.response) {
          const data = response.data.response;
          setPersonalInfo({
            name: `${data.lastName} ${data.firstName}`,
            email: data.email || '',
            phone: data.phoneNumber || '',
            gender: data.gender === 'WOMEN' ? 'Nữ' : data.gender === 'MEN' ? 'Nam' : data.gender || '',
            address: data.address ? data.address.join(', ') : '',
          });
        } else {
  ...
  const handleSavePersonal = () => {
    console.log('Đã lưu thông tin cá nhân:', personalInfo);
    alert('Thông tin cá nhân đã được lưu!');
    // Gọi API để lưu thông tin cá nhân (chưa triển khai)
  };
  ...
  return (
    <div className="container mx-auto p-6 bg-white shadow-md rounded-lg mt-10 text-xl" style={{ maxWidth: '1200px', padding: '50px' }}>
            ...
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                <input
                  ...
                  name="gender"
                  value="Khác"
                  checked={personalInfo.gender === 'Khác'}
                  onChange={handleGenderChange}
                />
